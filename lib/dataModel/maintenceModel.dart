...
class MaintainestListModel {
  String? status;
  String? message;
  List<MaintainestData>? data;

  MaintainestListModel({this.status, this.message, this.data});

  MaintainestListModel.fromJson(Map<String, dynamic> json) {
    status = json['status'];
    message = json['message'];
    if (json['data'] != null) {
...
class MaintainestData {
  ...
  MaintainestData.fromJson(Map<String, dynamic> json) {
    ...
    // Check if 'due_date' is not null and is a List
    if (json['due_date'] != null && json['due_date'] is List) {
      dueDate = List<String>.from(json['due_date']);
    } else {
      dueDate = []; // Assign an empty list if 'due_date' is null or not a List
    }

    // Check if 'done_date' is not null and is a List
    if (json['done_date'] != null && json['done_date'] is List) {
      doneDate = List<String>.from(json['done_date']);
    } else {
      doneDate = []; // Assign an empty list if 'done_date' is null or not a List
    }
